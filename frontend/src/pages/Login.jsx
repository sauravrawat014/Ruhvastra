...
export default function Login(){
    ...
    return(
        <form onSubmit={onSubmit} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 tex-gray-800'>
            <div className='inline-flex irems-center gap-2 mb-2 mt-10'>
                <p className='prata-regular text-3xl'>{currentState}</p>
                <hr className='border-none h-[1.5px] w-8 bg-gray-800 mt-4'/>
            </div>
            {currentState === 'Login'? '': 
             <input type='text' className='w-full px-3 py-2 border border-gray-800' placeholder='Name' onChange={(e)=>setName(e.target.value)} value={name} required/>}
             <input type='email' className='w-full px-3 py-2 border border-gray-800' placeholder='Email' onChange={(e)=>setEmail(e.target.value)} value={email} required/>
              ...
              <button className='bg-black text-white font-ligt px-8 py-2 mt-4'>{currentState === 'Login'? 'Sign In': 'Sign Up'}</button>
